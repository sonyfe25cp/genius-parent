...
<div id="cc">
<ol id="cco">
	<c:if test="${xnewslist!=null}">
		<c:forEach var="res" items="${xnewslist}">
		<li class="ccl">
		<div class="cclbs"></div>
		<div class="cclc">
...
<ul class="pagination">
	<li class="text">
		<c:if test="${pageNo!=1}">
			<a href="/xnews/list?pageNo=${pageNo-1}&pageSize=10">Previous</a>
		</c:if>
	</li>
	<c:forEach   var="num"   begin="${pb}"   end="${pe}"   step="1">   
		...
		<c:otherwise>
			<li><a href="/xnews/list?pageNo=${num}&pageSize=10" title="">${num}</a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	<li class="text">
		<c:if test="${pageNo!=totalP}">
			<a href="/xnews/list?pageNo=${pageNo+1}&pageSize=10" title="">Next</a>
		</c:if>
	</li>
</ul>
<div id="foot"></div>
