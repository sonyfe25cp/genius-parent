...
$(document).ready(function(){
    $(".delete").click(function(){
    	...
    	$.post("/admin/xnews/remove",{
        	...
        	//alert(data);
        	location.href ="/admin/hot/list-one/${term}";
       });
     });    
});
</script>
...
<body style="background: none">
	<div class="contentcontainer">
		...
		<div class="contentbox">
				...
				<tbody>
					...
					<c:forEach var="news" items="${list}">
						<tr>
							...
							<td><a href="http://${news.sourceHost}" target="_blank">${news.sourceHost}</a></td>
			...
			<ul class="pagination">
				<li class="text"><c:if test="${pageNo!=1}">
						<a href="/admin/hot/list-one/${term}?pageNo=${pageNo-1}">Previous</a>
					</c:if>
				</li>
				...
				<c:forEach var="num" begin="${pb}" end="${pe}" step="1">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="/admin/hot/list-one/${term}?pageNo=${num}" title="">${num}</a>
							</li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${pageNo!=totalP}">
					<li class="text"><a
						href="/admin/hot/list-one/${term}?pageNo=${pageNo+1}" title="">Next</a></li>
				</c:if>
