	...
	<div class="contentcontainer">
		...
		<div class="contentbox">
				...
				<tbody>
					...
					<c:forEach var="extract_cfg" items="${extract_cfgs}">
						<tr>
							...
							<td><%=((List<ExtractConfiguration>) request
						...
						.replace(">", "&gt;").replace("<", "&lt;")%></td>
							...
							<td><c:if test="${extract_cfg.enabled == true }">激活</c:if> <c:if
									test="${extract_cfg.enabled == false }">关闭</c:if></td>
			...
			<ul class="pagination">
				<c:if test="${pageNo != 1 }">
					<li class="text"><a
						href="/admin/extract/slist/${sname}?pageNo=${pageNo-1 }">Previous</a>
					</li>
				</c:if>
				<c:forEach var="num" begin="1" end="${total }" step="1">
					...
					<c:if test="${num != pageNo }">
						<li><a href="/admin/extract/slist/${sname}?pageNo=${num}"
							title="">${num }</a>
						</li>
					</c:if>
				</c:forEach>
				<c:if test="${pageNo != total }">
					<li class="text"><a
						href="/admin/extract/slist/${sname}?pageNo=${pageNo+1 }">Next</a>
					</li>
				</c:if>
			</ul>
